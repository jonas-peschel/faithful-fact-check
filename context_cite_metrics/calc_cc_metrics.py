...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, choices=["cnn_daily_mail", "druid", "averitec", "averitec_short_ans", "averitec_web_evidence", "averitec_web_evidence_short_ans", "multifieldqa_en"], required=True, help="Which dataset to use.")
...
def calc_top_k_log_prob_drop(cc: ContextCiter, res: dict, attr_methods: List[str], use_longcite: bool):
    ...
    Also calculate for k = #citations generated by the attribution method itself.
    """
    ...
    # top-k log-prob drop for k=1,3,5,k_longcite,k_self
    ...
    for attr_method in attr_methods:
        ...
        res["methods"][attr_method]["metrics"]["top_k_drop"]["top_k_drop_citations"] = []  # for k = #citations cited by the attribution method itself
        if use_longcite:
        ...
        for sent_idx, (start_idx, end_idx) in zip(range(len(answer_statements)), char_spans):
            ...
            if np.any([not np.any(scores) for k, scores in zip(ks, attr_scores_ks) if k != 0]):
                ...
                res["methods"][attr_method]["metrics"]["top_k_drop"]["top_k_drop_citations"].append(None)
                if use_longcite:
            ...
            for i,k in enumerate(ks[1:], start=1):
                ...
                if k == k_self:
                    res["methods"][attr_method]["metrics"]["top_k_drop"]["top_k_drop_citations"].append(diff.item())
